...
The aggregation step `pixi run reg` writes the selected XGBoostLSS and NGBoost distribution tables, synchronizes regression figures into `paper/plots/regression/`, and excludes numerically invalid candidate/dataset pairs rather than imputing missing folds. It refuses to create either fused baseline until every configured family result file exists. Eligibility requires exactly nine finite CRPS and RMSE folds. The incomplete XGBoostLSS candidates are Gaussian on `yacht_hydrodynamics` (8/9 CRPS folds), Student-t on `energy_efficiency` (8/9), Student-t on `parkinsons_updrs` (8/9), and Student-t on `yacht_hydrodynamics` (6/9). The locked NGBoost aggregate uses all five family sweeps: LogNormal is ineligible on `bike_sharing` and `concrete_strength` (8/9 CRPS and non-finite RMSE) and on `superconductor` (non-finite RMSE). The selected Exponential result on `forest_fires` is finite but catastrophically poor and is retained rather than clipped.
