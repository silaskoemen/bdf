...
class NormalMeanNormalGammaSkewNormal(BDFDistribution):
    ...
    def sample_prior(self, size: int, random_state: int) -> np.ndarray:
        ...
        """
        raise NotImplementedError(
            "Due to parameterization of global mean and prior on gamma, " "no fixed prior is available to sample from. "
        )
    ...
    def get_posterior_mean(self, *, data: np.ndarray | None = None, params: dict[str, float] | None = None) -> float:
        ...
        """
        if params is not None:
            posterior_alpha, posterior_xi, posterior_omega = (
                params.get("posterior_alpha", params.get("alpha")),
                params.get("posterior_xi", params.get("xi")),
                params.get("posterior_omega", params.get("omega")),
            )
            if posterior_alpha is None or posterior_xi is None or posterior_omega is None:
    ...
    def get_posterior_params(self, data: np.ndarray) -> dict:
        ...
        dict
            A dictionary containing the posterior parameters 'alpha', 'xi', and 'omega'.
        """
        posterior_alpha, posterior_xi, posterior_omega = self.calc_posterior_params(data)
        return {"alpha": posterior_alpha, "xi": posterior_xi, "omega": posterior_omega}
