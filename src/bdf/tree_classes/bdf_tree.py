...
class BDFTree:
    ...
    def fit(self, X: np.ndarray, y: np.ndarray, col_idcs: list | np.ndarray | None = None, verbose: bool = False, eta: float = 0.025) -> 'BDFTree':
        ...
        if len(y) >= self.min_samples_split:
            ...
            if loss_reduction > 0 and feature_idx is not None:
                heapq.heappush(split_candidates, (-loss_reduction, id(self.root), self.root, 
                            X, y, feature_idx, threshold, left_indices, right_indices))
        ...
        while split_candidates:
            ...
            if self.reg_beta + self.reg_lambda * (2 * self.n_leaves + 1) - nll_reduction <= 0:
                ...
                for child_node, child_X, child_y in [(node.left_node, X_left, y_left), (node.right_node, X_right, y_right)]:
                    # Only try splitting if we have enough samples
                    if len(child_y) >= self.min_samples_split and child_node.depth < self.max_depth:
                        ...
                        if c_loss_reduction > 0 and c_feat is not None:
                            heapq.heappush(split_candidates, 
                                        (-c_loss_reduction, id(child_node), child_node, child_X, child_y, 
                                        c_feat, c_thresh, c_left_idx, c_right_idx))
            else:
