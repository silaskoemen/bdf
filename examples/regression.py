...
# %%
print(
    f"MSE BDF: {np.mean((bdf.predict(X_test.values) - y_test.values) ** 2)} | MSE RF: {np.mean((rf.predict(X_test.values) - y_test.values) ** 2)}, MSE GB: {np.mean((gb.predict(X_test.values) - y_test.values) ** 2)}"
)
# %%
