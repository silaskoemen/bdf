    ...
    for pct in np.linspace(0.2, 0.8, 20):
        ...
        try:
            ...
            if total_nll < best_nll:
                ...
                best_params = {
                    'left': (k_left, lambda_left),
                    'right': (k_right, lambda_right),
                    'split_idx': split_idx
                }
        except:
            continue
...
ax.plot(x_range, weibull_min.pdf(x_range, c=k_left, scale=lambda_left), 
        'b-', linewidth=2, label=f'Weibull(k={k_left:.2f}, λ={lambda_left:.2f})')
ax.set_xlabel('y')
...
ax.plot(x_range, weibull_min.pdf(x_range, c=k_right, scale=lambda_right), 
        'r-', linewidth=2, label=f'Weibull(k={k_right:.2f}, λ={lambda_right:.2f})')
ax.set_xlabel('y')
ax.set_ylabel('Density')
...
ax.plot([all_theoretical.min(), all_theoretical.max()], 
...
ax.plot(x_range, weibull_min.pdf(x_range, c=k_left_mse, scale=lambda_left_mse), 
        'b-', linewidth=2, label=f'Weibull(k={k_left_mse:.2f}, λ={lambda_left_mse:.2f})')
ax.set_xlabel('y')
ax.set_ylabel('Density')
...
ax.plot(x_range, weibull_min.pdf(x_range, c=k_right_mse, scale=lambda_right_mse), 
        'r-', linewidth=2, label=f'Weibull(k={k_right_mse:.2f}, λ={lambda_right_mse:.2f})')
ax.set_xlabel('y')
ax.set_ylabel('Density')
...
plt.suptitle('Comparison: Weibull-based Split (top) vs MSE-based Split (bottom)', 
             fontsize=14, y=1.00)
plt.tight_layout()
plt.show()
...
print(f"MSE split NLL: {weibull_nll(left_y_mse, k_left_mse, lambda_left_mse) + weibull_nll(right_y_mse, k_right_mse, lambda_right_mse):.2f}")
# %%
