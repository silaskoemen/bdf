...
# Calculate posterior parameters
posterior_mean_alpha = np.sign(delta) * (np.abs(delta)/np.sqrt(1 - delta**2)) ** (1/3)
posterior_mean_alpha = n_samples/(n_samples + lambda_alpha) * posterior_mean_alpha + lambda_alpha/(n_samples + lambda_alpha) * prior_mean_alpha
...
posterior_mean_xi = posterior_mean - posterior_omega * posterior_mean_alpha / np.sqrt(1 + posterior_mean_alpha ** 2) * np.sqrt(2/np.pi)

print(f"Posterior estimates: xi={posterior_mean_xi:.4f}, omega={posterior_omega:.4f}, alpha={posterior_mean_alpha:.4f}")
print(f"True mean: {XI + OMEGA * (ALPHA / np.sqrt(1 + ALPHA**2)) * np.sqrt(2/np.pi):.4f}, data mean: {sample_mean:.4f}, posterior mean: {posterior_mean_xi + posterior_omega * (posterior_mean_alpha / np.sqrt(1 + posterior_mean_alpha**2)) * np.sqrt(2/np.pi):.4f}")
...
x = np.linspace(min(data_skew)-5, max(data_skew)+5, 1000)
plt.plot(x, stats.skewnorm.pdf(x, ALPHA, loc=XI, scale=OMEGA), 
         'k-', lw=2, label='True Skew Normal')
...
plt.plot(x, stats.norm.pdf(x, loc=sample_mean, scale=np.std(data_skew, ddof=1)),
         'b--', lw=2, label='Normal Approximation')
