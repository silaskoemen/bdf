...
The results in this appendix justify BDF's leaf prediction and split scoring under fixed candidate partitions and regularity conditions. Evidence-based prequential and Bayes-factor identities assume fixed likelihood and prior specifications; for Normal--Normal this includes a fixed, assumed-known observation variance. The implemented leaf-local variance gives a profile/EB score, while Beta--Bernoulli and Gamma--Poisson integrate their likelihood parameters fully at fixed prior hyperparameters. The results do not prove posterior inference over tree structures or global consistency of the adaptive greedy forest.
...
Let $S=-\log m$ for marginal likelihoods under fixed model specifications and independent child priors. Then
\[
\Delta S(s;\ell)
=
S(\mathcal{D}_\ell)
