...
\log \mathrm{BF}(s),
\]
where $\mathrm{BF}(s)$ is the Bayes factor in favor of splitting node $\ell$ according to $s$.
\end{proposition}
...
\[
C \le K\lfloor 1/\eta_{\mathrm{cut}}\rfloor .
\]
...
\]
The Laplace expansion remains centered at the pseudo-true parameter under standard misspecified likelihood theory. The curvature term in the posterior approximation is governed by the expected negative Hessian
\[
I(\theta^\star)
=
-\mathbb{E}_{f_0^\ell}
...
The standard posterior consistency theorem for parametric models therefore yields, for every $\epsilon>0$,
\[
\pi(
...
Since the prior is positive and continuous at $\theta_0$, the classical Bernstein--von Mises theorem gives
\[
\left\|
...
\nabla^2\log p_{\theta^\star}(Y),
\]
and prior positivity at $\theta^\star$, the misspecified Bernstein--von Mises theorem gives a Gaussian posterior approximation centered near the MLE with covariance
\[
...
\begin{theorem}[Per-node split consistency]
\label{thm:split_consistency}
Let $R_\ell$ be a fixed node region at depth $d$ with $n_\ell\to\infty$.
Assume the regularity conditions of Proposition~\ref{prop:leaf_concentration}, and assume the candidate set has bounded cardinality
\[
C \le K\lfloor 1/\eta_{\mathrm{cut}}\rfloor .
\]

\emph{Beneficial split.}
If a candidate split $s^\star$ produces child regions with
\[
n_{\ell_L}/n_\ell \to \rho\in(0,1)
\]
and strictly positive population gain
\[
\Delta_{\mathrm{pop}}(s^\star)>0,
\]
then
\[
...
\emph{Spurious split.}
If the unsplit model is correctly specified at node $\ell$, so that
$f_0^\ell=p_{\theta_0}$ for some $\theta_0\in\mathrm{int}(\Theta)$, and all candidate splits are population-equivalent, then
\[
