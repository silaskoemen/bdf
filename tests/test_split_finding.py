...
def test_split_finding_basic():
    ...
    feature_idx, threshold, loss, left_mask, right_mask = node._find_best_split_python(
        X, y, 1, 0.0, None
    )
...
def test_rust_python_split_equivalence():
    ...
    py_feature_idx, py_threshold, py_loss, py_left, py_right = node._find_best_split_python(
        X, y, 1, 0.0, None
    )
    ...
    assert py_threshold is None and rust_threshold is None or \
           abs(py_threshold - rust_threshold) < 1e-10
